...
const src = path.resolve(TMP_DIR, 'openai-cam.csv')
const dest = path.resolve(TMP_DIR, 'openai-cam-cfg.bin')
const mixinBin = path.resolve(TMP_DIR, 'openai-cam-mixin.bin')

const fillCSV = ({ssid, pwd, openaiKey,language}) => {
...
const mixinOpenaiCamNVS = function(option) {
  generateCSV(option)
...
const useOpenaiCamNVS = function(option) {
  generateCSV(option)
  if(os.platform() === 'darwin' || os.platform() === 'linux') {
    try {
      spawnSync('chmod', ['+x', NVS_PY])
